...
import { getScenarioList } from "../../../api/yxgl/scenario.js";
export default {
  components: {
    Table
  },
  data() {
    return {
      ...
      actions: [
        { type: "primary", value: "编辑剧本", click: this.goQuestion }
      ]
    };
  },
  created() {
  ...
  methods: {
    // 获取具有剧本的NPC列表
    fetchData() {
      getScenarioList(this.listQuery).then(response => {
    ...
    goQuestion(row) {
      ...
      this.$router.push({
        ...
        query: { userId: row.id, name: row.nickname }
      });
    }
    // 删除
    /* handleDelete(row){
