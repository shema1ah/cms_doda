...
现在是用权限来动态生成路由，会根据权限进行筛选，拿整个路由去过滤掉没有权限的，然后渲染，每次刷新也都会重新拉取权限进行，并没有把权限存放到状态机中。
vuex状态机中只存了要用的名字和是否已经登录
...
https://panjiachen.github.io/vue-element-admin-site/zh/guide/advanced/icon.html#生成图标库代码
